...
impl From<internal::Lecture> for Lecture {
	fn from(base: internal::Lecture) -> Self {
		...
		if rooms.is_empty() {
			...
			online = rooms.iter().any(|room| room.to_lowercase().contains("online"));
		}

		// EVENT TYPE
		let mut event_type = EventType::Lecture;
		{
			if base.end_time.signed_duration_since(base.start_time) == chrono::Duration::hours(10) && base.start_time.hour() == 8 && base.start_time.minute() == 0  {
				event_type = EventType::Holiday;
			}
			if let Some(name_begin) = name.split(" ").next() {
				if name_begin.ends_with("klausur") || name_begin == "prüfung" || name_begin == "prüfungswahl" {
